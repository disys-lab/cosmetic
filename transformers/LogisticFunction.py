...
class LogisticRegression(nn.Module):
    def __init__(self, size=0, length_transform_salt=0, beta=None, sample_size=None):
        super(LogisticRegression, self).__init__()
        self.length_transform_salt = length_transform_salt
        self.beta = beta
        self.sample_size = sample_size       # number of observations in the sample
        self.size=size
        self.hash_func = None
    ...
    def setup_proof(self, model, default_value, model_path, settings_path, compiled_model_path, vk_path, pk_path, py_run_args):
        ...
        torch.onnx.export(model, (current_value, salt, self.beta, self.sample_size), model_path,
    ...
    def dump_data_for_proof_gen(self, raw_value, salt,  data_path):
        ...
        sample_size_array = self._convert_tensor_to_float_array(sample_size)

        input_data_dict = dict(input_data=[raw_value_array, salt_array, beta_array, sample_size_array])
    ...
    def forward_dry(self, a, salt, use_witness_file=True):
        ...
        if use_witness_file:
            ...
            forward_result_vector = self.forward(a, salt, beta, sample_size)

        return forward_result_vector

    def forward_mfili(self, a, salt, beta, sample_size):
        beta = beta.flatten()
        coefs = beta.reshape(1, -1)
        N = sample_size[0, 0].reshape(1, -1)
        x_dim = coefs.shape[1]
        x = a[0:1, :x_dim]
        y = a[0, x_dim]
        eta = x @ coefs.T  # (1, 1)
        pred = torch.where(eta >= 0.0, 1.0, 0.0)  # shape (1,1)
        result = torch.where(pred == y, 1.0, 0.0) / N
        result = torch.cat([result, salt], dim=1)  # (1, num_rows + salt_dim)

    def forward(self, a, salt, beta, sample_size):
        beta = beta.flatten()

        #OLD VERSION
        # intercept = beta[0].reshape(1, 1)
        # coefs = beta[1:].reshape(1, -1)
        ...
        #ETA COMPUTE OLD VERSION
        # eta = x @ coefs.T + intercept  # (1, 1)

        #ETA COMPUTE NEW VERSION
        eta = x @ coefs.T  # (1, 1)

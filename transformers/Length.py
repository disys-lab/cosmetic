...
class Length(nn.Module):
    def __init__(self, size=0, length_transform_salt=0):
    ...
    def forward(self, a, salt,):
        ...
        result = torch.where(mask, torch.zeros_like(a), torch.ones_like(a))

        eta = result.max(dim=1, keepdim=True).values

        salt = salt.to(dtype=eta.dtype, )

        result = torch.cat([eta, salt], dim=1)

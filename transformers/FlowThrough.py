...
class FlowThrough(nn.Module):
    def __init__(self,size=0,length_transform_salt=0,atol=1e-3,rtol=1e-12):
        self.atol=atol
        self.rtol=rtol
        super(FlowThrough, self).__init__()
    ...
    def setup_proof(self, model, default_value, model_path, settings_path, compiled_model_path, vk_path, pk_path,py_run_args):
        ...
        torch.onnx.export(model, (current_value, salt,), model_path,
    ...
    def dump_data_for_proof_gen(self, raw_value, salt, data_path):
        ...
        # computed_result_array = self._convert_tensor_to_float_array(computed_result)

        input_data_dict = dict(input_data=[raw_value_array, salt_array,])
    ...
    def forward_dry(self,a,salt,use_witness_file=True):
        if use_witness_file:
            ...
            forward_result_vector = self.forward(a,salt)

        return forward_result_vector

    def forward(self,a,salt):

        #result = (torch.pow(a, 2) + 5.67)

        result = torch.cat([a, salt], dim=1)
